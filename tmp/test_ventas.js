...
async function run() {
    const res = await fetch('https://bizbazar-8hy2y0d41-ao-oj05s-projects.vercel.app/api/productos', { headers });
    const data = await res.json();
    const items = data.data || data;
    console.log(items.map(i => ({ nombre: i.nombre, categoria: i.categoria, estado: i.estado })));
}
run();
