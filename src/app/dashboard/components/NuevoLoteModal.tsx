            ...
            <div className="bg-white shadow-2xl w-full max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-300 rounded-2xl overflow-hidden">
                ...
                <div className="p-8 grid grid-cols-2 gap-6 text-left">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#475569]">Nombre del lote</label>
                        <input className="border border-slate-200 rounded-xl px-4 py-3.5 text-[15px] focus:outline-none focus:ring-1 focus:ring-slate-300 placeholder-slate-400" placeholder={isRopa ? "Ej: Lote Primavera 2026" : "Ej: Lote Joyería Premium"} value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#475569]">Cantidad de piezas</label>
                        <input type="number" className="border border-slate-200 rounded-xl px-4 py-3.5 text-[15px] focus:outline-none focus:ring-1 focus:ring-slate-300 placeholder-slate-400" placeholder="0" value={form.piezas} onChange={e => setForm(f => ({ ...f, piezas: e.target.value }))} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#475569]">Fecha de compra</label>
                        <input type="date" max={TODAY} className="border border-slate-200 rounded-xl px-4 py-3.5 text-[15px] focus:outline-none focus:ring-1 focus:ring-slate-300 text-slate-700" value={form.fecha} onChange={e => setForm(f => ({ ...f, fecha: e.target.value }))} />
                    </div>
                    
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#475569]">Gastos adicionales</label>
                        <div className="relative flex flex-col gap-1">
                            <span className="absolute left-4 top-3.5 text-slate-400 text-[15px] pointer-events-none">$</span>
                            <input type="number" className="w-full border border-slate-200 rounded-xl pl-8 pr-4 py-3.5 text-[15px] focus:outline-none focus:ring-1 focus:ring-slate-300 placeholder-slate-400" placeholder="0.00" value={form.gastosAdicionales} onChange={e => setForm(f => ({ ...f, gastosAdicionales: e.target.value }))} />
                            <span className="text-[11px] text-slate-400 mt-1">Transporte, bolsas, etc.</span>
                        </div>
                    </div>

                    <div className="flex flex-col gap-1.5 col-span-2">
                        <label className="text-[13px] font-semibold text-[#475569]">{isRopa ? 'Precio total del lote' : 'Inversión total del lote'}</label>
