...
import { Search, Gavel, Clock, ChevronRight, X } from "lucide-react";
...
interface SubastaItem {
    ...
    tiempoRestante: number; // in seconds, just to simulate
    pujas: number;
}

const INITIAL_SUBASTAS: SubastaItem[] = [
    { id: '1', nombre: 'Pulsera oro rosa premium', codigo: 'JY15', categoria: 'Joyería', imagen: 'https://images.unsplash.com/photo-1611591437281-460bfbe1220a?w=400&q=80', precioBase: 200, pujaActual: 250, estado: 'Activa', tiempoRestante: 3600, pujas: 5 },
    { id: '2', nombre: 'Vestido de noche exclusivo', codigo: 'RO-42', categoria: 'Ropa', imagen: 'https://images.unsplash.com/photo-1539008835657-9e8e9680c956?w=400&q=80', precioBase: 150, pujaActual: 320, estado: 'Activa', tiempoRestante: 7200, pujas: 12 },
    { id: '3', nombre: 'Anillo diamante vintage', codigo: 'JY-88', categoria: 'Joyería', imagen: 'https://images.unsplash.com/photo-1605100804763-247f67b454d6?w=400&q=80', precioBase: 500, pujaActual: 800, estado: 'Cerrada', tiempoRestante: 0, pujas: 18 },
];
...
export default function SubastasPage() {
    ...
    const [subastas, setSubastas] = useState<SubastaItem[]>(INITIAL_SUBASTAS);
    ...
    const registrarPuja = (id: string, monto: number) => {
        setSubastas(prev => prev.map(s => {
            if (s.id === id && s.estado === 'Activa' && monto > s.pujaActual) {
                return { ...s, pujaActual: monto, pujas: s.pujas + 1 };
            }
            return s;
        }));
        if (selectedSubasta && selectedSubasta.id === id) {
            setSelectedSubasta(prev => prev ? { ...prev, pujaActual: monto, pujas: prev.pujas + 1 } : null);
        }
    };

    const cerrarSubasta = (id: string) => {
        setSubastas(prev => prev.map(s => {
            if (s.id === id) {
                return { ...s, estado: 'Cerrada', tiempoRestante: 0 };
            }
            return s;
        }));
        if (selectedSubasta && selectedSubasta.id === id) {
            setSelectedSubasta(prev => prev ? { ...prev, estado: 'Cerrada', tiempoRestante: 0 } : null);
        }
    };
            ...
            <div className={cn("flex-1 transition-all duration-300", isCollapsed ? "ml-20" : "ml-64")}>
                ...
                <main className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filtered.map(s => (
                            <div key={s.id} onClick={() => setSelectedSubasta(s)} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden cursor-pointer hover:shadow-lg transition-all group flex flex-col">
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            id="pujainput"
                                            type="number"
                                            defaultValue={selectedSubasta.pujaActual + 10}
                                        ...
                                        <button
                                            onClick={() => {
                                                const val = parseFloat((document.getElementById('pujainput') as HTMLInputElement).value);
                                                if (val > selectedSubasta.pujaActual) {
                                                    registrarPuja(selectedSubasta.id, val);
                                                }
                                            }}
                                            className="bg-[#40C4AA] hover:bg-[#40C4AA]/90 text-white font-bold px-6 rounded-xl transition-colors shadow-lg shadow-teal-200 flex items-center gap-2"
                                        >
                                            Pujar
                                    ...
                                    <button
                                        ...
                                        className="w-full mt-2 py-3 bg-red-50 text-red-600 hover:bg-red-100 font-bold rounded-xl transition-colors"
                                    >
